...
router.route('/beneficiary-financing').post((req, res) => {
    if(!req.header('Authorization')){ 
        res.status(400).json({ data: { status: false, code: 400, message: 'Required authorization header not found.' } })
        return;
    }else{
        operationValrepBeneficiaryFinancing(req.header('Authorization'), req.body).then((result) => {
            if(!result.status){ 
                res.status(result.code).json({ data: result });
                return;
            }
            ...
            res.status(500).json({ data: { status: false, code: 500, message: err.message, hint: 'operationValrepBeneficiaryFinancing' } });
        });
    }
});

const operationValrepBeneficiaryFinancing = async(authHeader, requestBody) => {
    if(!helper.validateAuthorizationToken(authHeader)){ return { status: false, code: 401, condition: 'token-expired', expired: true }; }

    let query = await bd.beneficiaryFinancingValrepQuery().then((res) => res);
    if(query.error){ return { status: false, code: 500, message: query.error }; }
    let jsonArray = [];
    let nombre; 
    for(let i = 0; i < query.result.recordset.length; i++){
        if(query.result.recordset[i].XNOMBRE && query.result.recordset[i].XAPELLIDO){
            nombre = query.result.recordset[i].XNOMBRE + ' ' + query.result.recordset[i].XAPELLIDO
        }else{
            nombre = query.result.recordset[i].XNOMBRE
        }
