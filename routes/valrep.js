...
const operationValrepParentPolicy = async(authHeader, requestBody) => {
    ...
    let jsonArray = [];
    for(let i = 0; i < query.result.recordset.length; i++){
        jsonArray.push({ ccarga: query.result.recordset[i].CCARGA, xdescripcion: query.result.recordset[i].XDESCRIPCION_L, xpoliza: query.result.recordset[i].XPOLIZA, fcreacion: query.result.recordset[i].FCREACION });
    }
    return { status: true, list: jsonArray }
}
